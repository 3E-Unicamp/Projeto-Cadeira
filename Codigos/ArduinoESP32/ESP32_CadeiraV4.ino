...
V3 ~ 25/10/2023 : Nesta versão crio um primeiro protótipo da funcionalidade do ULP e junto ele com 
                  ...
                  no código.
V4 ~ 27/10/2023 : Nesta versão ajeitei o código para funcionar com o ESP32 MINI 32 V1.3.
...
#define PINO_EIXO_X GPIO_NUM_4
#define PINO_EIXO_Y GPIO_NUM_2
...
volatile uint8_t status_gpio33 = 0;
volatile uint8_t status_gpio34 = 0;
volatile uint8_t status_gpio35 = 0;
...
const int lcdPWMPINX = RTCIO_GPIO4_CHANNEL + 14;
const int lcdPWMPINY = RTCIO_GPIO2_CHANNEL + 14;
...
void IRAM_ATTR Timer1_ISR(){
  if(digitalRead(GPIO_NUM_34) == 1){
    num_botao = GPIO_NUM_34;
    status_gpio33 = 0;
    status_gpio34 = 1;
    status_gpio35 = 0;
  } else if(digitalRead(GPIO_NUM_33) == 1){
    num_botao = GPIO_NUM_33;
    status_gpio33 = 1;
    status_gpio35 = 0;
    status_gpio34 = 0;
  } else if(digitalRead(GPIO_NUM_35) == 1){
    num_botao = GPIO_NUM_35;
    status_gpio33 = 0;
    status_gpio34 = 0;
    status_gpio35 = 1;
  } else {
...
void loop(){
  ...
  if (wakeup_reason != ESP_SLEEP_WAKEUP_ULP){
    ...
    if((status_gpio33 == 1)||(status_gpio34 == 1)||(status_gpio35 == 1)){
      num_t0s = 0;
    }
...
void init_ulp(void){
  ...
  const ulp_insn_t  ulp_blink[] = {
    M_LABEL(1),
      ...
      I_RD_REG(RTC_GPIO_IN_REG, 18, 18), // R0 = digitalRead(GPIO_NUM_25)
      I_DELAY(25535),
      M_BGE(2, 1),
      I_RD_REG(RTC_GPIO_IN_REG, 19, 19),
      I_DELAY(25535),
      M_BGE(3, 1),
      I_RD_REG(RTC_GPIO_IN_REG, 22, 22),
      I_DELAY(25535),
      M_BGE(4, 1),
  ...
  rtc_gpio_set_level(GPIO_NUM_35, 0);
